    ...
    <div className="w-full py-10 bg-white border-y border-slate-100 overflow-hidden relative z-20">
      ...
      <div className="relative w-full overflow-hidden flex items-center py-2 bg-slate-50/50">
        ...
        <div className="animate-marquee animate-marquee-hover-pause flex items-center gap-6">
          {scrollingBrands.map((brand, index) => (<div key={index} className="flex items-center gap-3 bg-white border border-slate-200/80 px-2 py-2 rounded-xl shadow-sm hover:border-sky-400 hover:shadow-md hover:shadow-sky-500/5 transition-all duration-300 shrink-0 select-none group cursor-pointer">
            <div className="h-10 w-10 shadow rounded-lg overflow-hidden group-hover:bg-sky-50 transition-all duration-300">
              <img className="h-full w-full rounded-lg p-1 object-contain" src={new URL(`../assets/brands/${brand.icon}`, import.meta.url).href} alt={`${brand.name} logo`} />
            </div>
