  ...
  const brands = [
    {
      ...
      icon: <Video className="h-4.5 w-4.5 text-primary" />
    },
    {
  ...
  return (<div className="w-full py-10 bg-white border-y border-slate-100 overflow-hidden relative z-20">
    ...
    <div className="relative w-full overflow-hidden flex items-center py-2 bg-slate-50/50">
      ...
      <div className="animate-marquee animate-marquee-hover-pause flex items-center gap-6">
        {scrollingBrands.map((brand, index) => (<div key={index} className="flex items-center gap-3 bg-white border border-slate-200/80 px-5 py-3 rounded-xl shadow-sm hover:border-sky-400 hover:shadow-md hover:shadow-primary/5 transition-all duration-300 shrink-0 select-none group cursor-pointer">
          <div className="p-1.5 bg-slate-50 rounded-lg group-hover:bg-sky-50 transition-all duration-300">
            {brand.icon}
          </div>
          <div className="flex flex-col text-left">
            <span className="font-sans font-bold text-xs text-slate-800 tracking-wide group-hover:text-primary transition-colors duration-200">
              {brand.name}
            </span>
