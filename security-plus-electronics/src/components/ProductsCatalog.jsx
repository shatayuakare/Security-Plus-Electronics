  ...
  return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
    <section className="py-16 px-8 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto text-sans">
        ...
        {/* Animated Brand Partners Carousel */}
        <div className="mb-12">
          <BrandCarousel />
        </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-10 bg-white border border-slate-200/80 p-5 rounded-2xl shadow-sm">
          ...
          <div className="flex items-center gap-2.5 w-full lg:w-auto self-stretch lg:self-auto shrink-0 justify-end">
            ...
            <select value={productSortOption} onChange={(e) => setProductSortOption(e.target.value)} className="bg-white border border-slate-200 hover:border-primary text-slate-800 text-xs px-3.5 py-2.5 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary font-sans cursor-pointer transition-colors shadow-sm w-full lg:w-auto min-w-[180px]">
        ...
        {/* Grid with staggered, smooth entrance animations */}
            ...
            return sorted.map((product) => (<motion.div layout variants={staggerItem} key={product.id} whileHover={{
              ...
              <div>
                ...
                <div className="aspect-video bg-slate-100 border border-slate-100 rounded-xl mb-4 relative group-hover:border-sky-200 transition-colors overflow-hidden flex items-center justify-center">
                  ...
                  {product.isNewArrival && (<span className="absolute top-2.5 left-2.5 text-[8px] font-extrabold bg-primary text-white px-2.5 py-1 rounded-md uppercase tracking-wider z-20 shadow-sm">
                    New Arrival
                  </span>)}
                ...
                {/* Title */}
                <h3 className="text-sm font-bold text-slate-900 uppercase group-hover:text-primary transition-colors">
                  {product.name}
                </h3>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center z-20">
                ...
                <div className="flex gap-1.5">
                  ...
                  }} className="bg-slate-50 hover:bg-primary border border-slate-200 hover:border-primary text-slate-700 hover:text-white px-3 py-1.5 text-[9px] font-bold uppercase transition-all duration-300 rounded-xl flex items-center gap-1 cursor-pointer shadow-sm">
                    <ShoppingBag className="h-3.5 w-3.5 text-primary hover:text-white" />
                    Purchase
                  </button>
                </div>
              </div>
